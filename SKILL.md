...
description: Diagnose and fix Windows Codex Desktop reconnect loops or direct-connection failures by enabling Codex's native system-proxy support. Use when Codex shows repeated reconnect attempts despite a working Clash, FlClash, or other Windows system proxy. Do not use for general Windows network changes.
---
...
5. Verify `codex features list` reports `respect_system_proxy ... true`.
6. Tell the user to fully exit Codex from the system tray once and reopen it from their original desktop, Start menu, or taskbar entry.
