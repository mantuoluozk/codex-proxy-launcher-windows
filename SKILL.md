...
description: Diagnose and fix Windows Codex Desktop reconnect loops, direct-connection failures, and phone Remote connection failures by enabling Codex's native system-proxy support. Use when Codex or Remote fails despite a working Clash, FlClash, or other Windows system proxy. Do not use for general Windows network changes.
---
...
6. If `RestartRequired` is true, tell the user to fully exit Codex from the system tray once and reopen it from their original desktop, Start menu, or taskbar entry before testing phone Remote.
7. If Remote still fails after that restart, inspect only logs created after the new process start. Distinguish an authentication or HTTPS/WebSocket proxy failure from a transport-specific failure before proposing TUN mode.
...
Switching proxy nodes behind the same local listener requires no Codex change, including for phone Remote. If the proxy application or local port changes, Codex may need to be restarted so it can read the updated Windows setting.
